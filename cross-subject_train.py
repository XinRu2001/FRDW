...
import random
import argparse
from utils import get_data, EEGNet, Transformer,common_spatial_pattern, EA
...
def mod_train(X_TRAIN, Y_TRAIN, X_VALID, Y_VALID,model_save_path):
    print("=========find the best epoch and model===========")
...
def data_and_label(dataset="004-2014"):
    print("=========source domain data===========")
    persons=[1,2,3,4,5,6,7,8,9]
    source_person = []
    for i in persons:
        if not personid == i:
            source_person.append(i)
    data_X_train_all = []
    data_X_evl_all = []
    data_Y_train_all = []
    data_Y_evl_all = []
    for xh,i in enumerate(source_person):
        print("------------------------", "subject:", i)

        if args.dataset =="004-2014":
            file_path = current_working_dir + "/datasets/004-2014/" + "B0" + str(i) + "T.mat"
        ...
        elif args.dataset =="001-2014" and CLASSES_NUM==2:
            file_path = current_working_dir + "/datasets/001-2014/" + "A0" + str(i) + "T.gdf"
        ...
        elif args.dataset == "001-2014" and CLASSES_NUM == 4:
            file_path = current_working_dir + "/datasets/001-2014/" + "A0" + str(i) + "T.gdf"
        ...
        if args.if_EA:
            data_X_train, _ = EA.EA(data_X_train)
            data_X_evl, _ = EA.EA(data_X_evl)

        if xh == 0:
            data_X_train_all = data_X_train
            data_X_evl_all = data_X_evl
            data_Y_train_all = data_Y_train
            data_Y_evl_all = data_Y_evl
        else:
            data_X_train_all = np.vstack((data_X_train_all, data_X_train))
            data_X_evl_all = np.vstack((data_X_evl_all, data_X_evl))
            data_Y_train_all = np.concatenate((data_Y_train_all, data_Y_train))
            data_Y_evl_all = np.concatenate((data_Y_evl_all, data_Y_evl))

    return data_X_train_all,data_Y_train_all,data_X_evl_all,data_Y_evl_all
...
if __name__=='__main__':
    ...
    parser.add_argument('--if_EA', type=str,default=True ,help="Choose if EA,yes for sure,others for no")
