...
#动态窗实现
def dynamic_window_conv(data,sqrtRefEA, min_len, max_len, personid, theta,model_file):
    """
    ...
    for len in lens:
        pre_data = data[:, :len]
        pre_data = np.dot(sqrtRefEA, pre_data)
        label, output_prob = result_go.recognize(data=pre_data, personID=personid, model_file=model_file)  # 0 1 2 3
    ...
    trust_data = np.dot(sqrtRefEA, trust_data)
    label, output_prob = result_go.recognize(data=trust_data, personID=personid, model_file=model_file)  # 0 1 2 3
    ...
    if args.dataset == "001-2014" and class_num == 4:
        ...
        for xuhao, type_mi in enumerate(event_type):
            if type_mi == '783':
                ...
                if total_num<=10:
                    ...
                    dynamic_len.append(args.train_length)
                if total_num==10:
                ...
                if total_num>=10:
                    ...
                    result,datalen = dynamic_window_conv(data_what,sqrtRefEA,dynamic_window_min_len,dynamic_window_max_len,personid,theta_what,args.modelEA_save_path)
                    result = int(result)  # 0123
                    label_now = int(np.array(true_labels['classlabel'][total_num - 1]))
        ...
        for xuhao, event_position in enumerate(TRIAL):
            ...
            if total_num >= 10:
                event_start_position = int((event_position - 1) + fs * 4)
                data_what_all = X[:, (event_start_position):(event_start_position + args.train_length)]
                result,datalen = dynamic_window_conv(data_what_all,sqrtRefEA,dynamic_window_min_len,dynamic_window_max_len,personid,theta_what,args.modelEA_save_path)
                label_now = int(np.array(Y[xuhao]))
                if result + 1 == label_now:
                    correct_num = correct_num + 1
                dynamic_len.append(datalen)
