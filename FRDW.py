...
class dynamic_padding():
    ...
    def pading_copy_before(self,data,finnal_len):
        """
        FR
        """
        if data.shape[3] < finnal_len - data.shape[3]:
            while 1:
                data = torch.cat((data, data), dim=3)
                if data.shape[3] >= finnal_len:
                    break
            padding_data = data[:,:,:,:finnal_len]
        else:
            padding_data = torch.cat((data, data[:,:,:,:finnal_len - data.shape[3]]), dim=3)
        return padding_data
...
class get_result():
    def __init__(self,model_number,length,seedval):
        ...
        self.cal_len = length
        self.seeder=seedval
        self.pading_method = dynamic_padding()
    ...
    def getModel(self, personID,C):

        model = {}  # 创建当前用户的模型字典
        for i in range(self.model_num):
            seed = self.seeder + i
            model_path = current_working_dir + args.model_save_path + str(personID) +'_' + str(seed) + '.pth'
    ...
    def recognize(self, data, personID):
        ...
        if args.model_type=='EEGNet': 
            ...
            if data.shape[3] != raw_length:
                p_data=self.pading_method.pading_copy_before(data, raw_length)
            else:
                p_data = data
            
            mod = self.getModel(personID,C)
            ...
            data = self.band_Filter(data, personID)
            data = np.expand_dims(data, 0)
            if data.shape[2] != raw_length:
                if data.shape[2] < raw_length - data.shape[2]:
                    while 1:
                        data = np.concatenate((data, data), axis=2)
                        if data.shape[2] >= raw_length:
                            break
                    p_data = data[ :, :, :raw_length]
                else:
                    p_data = np.concatenate((data, data[ :, :, :raw_length - data.shape[2]]), axis=2)
            else:
                p_data = data
            mod = joblib.load(current_working_dir + args.model_save_path + str(personID) + ".m")
            csp_data=mod[0].transform(p_data)
...
def dynamic_window(data,min_len,max_len,personid,theta):
    """
    :param data: 每个trial的完整测试数据
    :param min_len: 动态窗最小窗口
    :param max_len: 动态窗最大窗口
    lap :窗口滑动的间隔
    :param personid: 用户id
    :param theta: 预测概率置信度阈值
    :return: 输出标签，输出概率
    """
    lap=10
    lens=list(range(min_len,max_len,lap))
    for len in lens:
        pre_data = data[:,:len]
        label, output_prob= result_go.recognize(data=pre_data,personID=personid) #0 1 2 3
        if output_prob >= theta:
            finnal_result = label
            datalen = len
            return finnal_result, datalen
            break
    trust_data = data[:, :max_len]
    label, output_prob = result_go.recognize(data=trust_data, personID=personid)  # 0 1 2 3
...
if __name__ == '__main__':
    ...
    if args.dataset == "001-2014" and class_num == 4:
        ...
        dynamic_len = []
        # 动态窗起始长度
        dynamic_window_min_len = 60
        dynamic_window_max_len = 100
        # 阈值theta
        theta_what = 0.6
        ...
        for xuhao, type_mi in enumerate(event_type):
            if type_mi == '783':
                ...
                data_what = (chan_time_all[:, (event_start_position + fs):(event_start_position + fs + args.train_length)])
                result, datalen = dynamic_window(data_what, dynamic_window_min_len,
                                                                dynamic_window_max_len, personid, theta_what)
                result = int(result)  # 0123
                label_now = int(np.array(true_labels['classlabel'][total_num - 1]))
                if (result + 1) == label_now:
                    correct_num = correct_num + 1
                dynamic_len.append(datalen)
        mean_len = np.mean(dynamic_len)
        acc=np.round(np.array(correct_num/total_num), decimals=4)
        finnal_ITR = itr_cal(correct_num / total_num,mean_len, class_num)
        print("acc:", np.round(np.array(acc), decimals=4))
        print("ITR:", np.round(np.array(finnal_ITR), decimals=4))
        print(mean_len)
        ...
        finnal_ITR = itr_cal(correct_num/total_num,args.train_length,class_num)

        mean_len = np.mean(dynamic_len)
        acc=np.round(np.array(correct_num/total_num), decimals=4)
        finnal_ITR = itr_cal(correct_num / total_num,mean_len, class_num)
        print(np.round(np.array(correct_num/total_num), decimals=4))
        ...
        for block_num in range(block_nums):
            ...
            X = all_data["data"][0, block_num]["X"][0, 0][:, :3].T  # X(point,channle) data
            Y = all_data["data"][0, block_num]["y"][0, 0][:, 0]  # y(0,trial) class
            TRIAL = all_data["data"][0, block_num]["trial"][0, 0][:, 0]
            for xuhao, event_position in enumerate(TRIAL):
                ...
                result, datalen = dynamic_window(data_what_all, dynamic_window_min_len,
                                                                dynamic_window_max_len, personid, theta_what)
